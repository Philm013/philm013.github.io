...
const editor = {
    ...
    bindBlockEvents() {
        ...
        blocks.forEach(el => {
            ...
            el.addEventListener('touchstart', (e) => {
                this.handleBlockMouseDown(e.touches[0], el);
            }, { passive: true });
    ...
    handleMouseMove(e) {
        ...
        if (this.isDragging) {
            ...
            // Snapping logic
            if (e.shiftKey || app.state.gridSnapping) {
                const snX = Math.round(newX / snap) * snap;
                const snY = Math.round(newY / snap) * snap;
                if ((snX !== block.x || snY !== block.y) && window.navigator.vibrate) {
                    // window.navigator.vibrate(5); // Optional: very subtle snap feedback
                }
                newX = snX;
                newY = snY;
            }
