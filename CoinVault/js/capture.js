...
export const Capture = {
    ...
    bindEvents() {
        ...
        if (toggleAutoBtn) {
            toggleAutoBtn.addEventListener('click', () => {
                ...
                toggleAutoBtn.classList.toggle('bg-amber-600', this.isAutoDetecting);
                toggleAutoBtn.classList.toggle('bg-black/40', !this.isAutoDetecting);
                const status = document.getElementById('detectionStatus');
                if (status) status.textContent = this.isAutoDetecting ? "Scanning for coins..." : "Auto-detect off";
            });
        }
    },
    ...
    loadUploadedImage(file) {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                this.detectCircles();
                if (status) status.textContent = "Tap a coin to auto-detect, or draw a circle.";
                this.redraw();
            };
    ...
    redraw() {
        ...
        if (this.currentImage) {
            ...
            this.ctx.beginPath();
            this.ctx.arc(this.canvasEl.width/2, this.canvasEl.height/2, 200, 0, Math.PI*2);
            this.ctx.stroke();
        }
        ...
        this.boxes.forEach(box => {
            ...
            this.ctx.arc(cx, cy, box.w/2, 0, Math.PI * 2);
            this.ctx.stroke();
