/* global d3 */
import { ui, toast, Modal } from './ui.js';
...
export function initGraph(callbacks) {
    ...
    simulation = d3.forceSimulation()
        ...
        .force("collision", d3.forceCollide().radius(d => Math.max(d.width || 60, d.height || 40) / 2 + 15))
    ...
    svg.on('click', (e) => {
        if (e.target === svg.node()) {
            if (interactionCallbacks.onCanvasClick) interactionCallbacks.onCanvasClick(e);
        }
    });
}
...
export function addNode(label, parentId = null, options = {}) {
    ...
    if (coords) {
        ...
        if (parentNode) {
            // Position in a circular offset from parent
            const angle = Math.random() * Math.PI * 2;
            const distance = 120 + Math.random() * 60;
            x = parentNode.x + Math.cos(angle) * distance;
            y = parentNode.y + Math.sin(angle) * distance;
        } else {
            // Position near center with a larger random spread
            x = (width / 2 - transform.x) / transform.k + (Math.random() * 200 - 100);
            y = (height / 2 - transform.y) / transform.k + (Math.random() * 200 - 100);
        }
    }
    ...
    // Briefly fix the position, then let it breathe
    setTimeout(() => {
        const n = nodes.find(node => node.id === newNode.id);
        if (n && currentLayout !== 'mindmap' && currentLayout !== 'flowchart') {
            delete n.fx;
            delete n.fy;
    ...
    updateGraph();
    if (interactionCallbacks.onGraphChanged) interactionCallbacks.onGraphChanged();
    return newNode;
}
...
export async function groupSelectedNodes() {
    ...
    const groupName = await Modal.prompt("Create Group", "Enter a name for the new group:", "New Group");
    if (!groupName) return;
...
/**
 * Zooms and pans the graph to fit all nodes within the visible area.
 */
export function fitView() {
    if (nodes.length === 0 || currentLayout === 'sunburst') return;
    ...
    const contentWidth = xExtent[1] - xExtent[0] + padding * 2;
    const contentHeight = yExtent[1] - yExtent[0] + padding * 2;
    ...
    d3.select("#graph-canvas").transition()
        .duration(750)
...
function calculateNodeDimensions() {
    nodes.forEach(d => {
        ui.textMeasurer.style.width = d.label.length > 30 ? '200px' : 'auto';
        ui.textMeasurer.textContent = d.label;
        const rect = ui.textMeasurer.getBoundingClientRect();
        d.width = Math.max(80, rect.width + 32);
        d.height = Math.max(60, rect.height + 48); // Extra space for header/footer
    });
}
...
function renderForceGraph(reapplyLayout = false) {
    ...
    linkEnter.append("path")
        .attr("class", "link-interaction-area")
        .on('dblclick', async (e, d) => {
            e.stopPropagation();
            const newLabel = await Modal.prompt("Edit Connection", "Enter a label for this relationship:", d.label || "");
            if (newLabel !== null) {
                d.label = newLabel;
                updateGraph();
            }
    ...
    labelGroup.append("text").attr("class", "link-label").text(d => d.label || '');
    
    link = linkEnter.merge(link);
    link.select('.link-label').text(d => d.label || '');
    link.select('.link-label-bg').attr("opacity", d => d.label ? 0.8 : 0);
    link.select('.link')
        .style("stroke", d => d.color || '#b2bec3')
        .style("stroke-dasharray", d => d.dashed ? '5,5' : 'none');
    ...
    const nodeCard = foreignObject.append('xhtml:div')
        .attr('class', 'node-card-container')
        .style('width', '100%')
        .style('height', '100%')
        .style('display', 'flex')
    ...
    nodeCard.append('xhtml:div')
        .attr('class', 'node-card-header')
        .style('height', '8px')
        .style('width', '100%')
        .style('background', d => d.color || 'var(--accent-primary)');

    // Node Content
    nodeCard.append('xhtml:div')
        .attr('class', 'node-card-body')
        .style('padding', '10px')
        .style('flex-grow', '1')
        .style('display', 'flex')
    ...
    // Node Footer/Actions
    const nodeFooter = nodeCard.append('xhtml:div')
        .attr('class', 'node-card-footer')
        .style('padding', '4px 8px')
        .style('display', 'flex')
        .style('justify-content', 'flex-end')
        .style('gap', '4px')
        .style('background', 'rgb(0 0 0 / 0.02)');

    nodeFooter.append('xhtml:span')
        .attr('class', 'iconify')
        .attr('data-icon', 'solar:magic-stick-3-bold-duotone')
        ...
        .on('click', (e, d) => {
            e.stopPropagation();
            interactionCallbacks.onNodeExpand?.(d);
        });
    ...
    node.select('.node-foreign-object').attr('x', d => -d.width / 2).attr('y', d => -d.height / 2).attr('width', d => d.width).attr('height', d => d.height);
    node.select('.node-card-body').html(d => d.label);
    node.select('.node-card-header').style('background', d => d.color || 'var(--accent-primary)');
...
const addHandleDrag = d3.drag()
    ...
    .on('drag', function(e, d) {
        const [gx, gy] = d3.pointer(e, g.node());
        g.select('.connector-preview')
            .attr('x2', gx)
            .attr('y2', gy);
        
        // Highlight potential targets
        const dropTargetEl = e.sourceEvent.target.closest('.node');
        g.selectAll('.node').classed('drag-target', false);
        if (dropTargetEl) {
            const dropTargetData = d3.select(dropTargetEl).datum();
            if (dropTargetData.id !== d.id) {
                d3.select(dropTargetEl).classed('drag-target', true);
            }
        }
    })
    .on('end', function(e, d) {
        g.selectAll('.link-interaction-area').style('pointer-events', 'auto');
        g.select('.connector-preview').remove();
        g.selectAll('.node').classed('drag-target', false);
